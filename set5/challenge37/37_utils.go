...
import (
	...
	"github.com/stripedpajamas/cryptopals/set5/challenge36"
)

func Start(email, password []byte) (challenge36.SRPServer, challenge36.SRPClient) {
	client := challenge36.SRPClient{}
	server := challenge36.SRPServer{}
...
func GetClientEmailPayload(c *challenge36.SRPClient, a *big.Int, email []byte) [][]byte {
	// get public A and make a payload with [email, A]
	return [][]byte{
		email,
		c.GetPublic(a).Bytes(),
	}
...
func GetServerSaltPayload(s *challenge36.SRPServer, b *big.Int) [][]byte {
	// get public B and make a payload with [salt, B]
	return [][]byte{
		s.Salt.Bytes(),
		s.GetPublic(b).Bytes(),
	}
...
func GetClientAuthPayload(c *challenge36.SRPClient) [][]byte {
	// returns [hmac(key, salt)]
	hash := hmac.New(sha256.New, c.S.Bytes())

	return [][]byte{
		hash.Sum(c.Salt.Bytes()),
	}
}

func ServerCheckAuth(s *challenge36.SRPServer, clientAuth []byte) bool {
	hash := hmac.New(sha256.New, s.S.Bytes())
	serverAuth := hash.Sum(s.Salt.Bytes())

	return hmac.Equal(serverAuth, clientAuth)
}
