...
func Start(email, password []byte) (SRPServer, SRPClient) {
	client := SRPClient{}
	server := SRPServer{}
...
func GetClientEmailPayload(c *SRPClient, a *big.Int, email []byte) [][]byte {
...
func GetServerSaltPayload(s *SRPServer, b, u *big.Int) [][]byte {
	// get public B and make a payload with [salt, B, u]
	return [][]byte{
		s.Salt.Bytes(),
		s.GetPublic(b).Bytes(),
		u.Bytes(),
...
func GetClientAuthPayload(c *SRPClient) [][]byte {
	// returns [hmac(key, salt)]
	hash := hmac.New(sha256.New, c.K[:])
...
func ServerCheckAuth(s *SRPServer, clientAuth []byte) bool {
	hash := hmac.New(sha256.New, s.K[:])
